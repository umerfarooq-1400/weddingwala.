...
// Vercel Par EROFS (Read-Only) Error Se Bachne Ke Liye Safe Check
try {
  ...
  console.log("Vercel environment: Skipping local disk directory creation");
}
...
async function connectDB() {
  ...
  try {
    ...
    console.log("DB Warning: Using sample data fallback");
  }
}
...
app.get('/api/venues', async (req, res) => {
  try {
    ...
    if (!venues || venues.length === 0) venues = sampleVenues;
    res.json(venues);
  } catch (err) {
